...
- `main.py` — entry point: load config, set up logging, start one `WeComClient` per group (env mode), or run the database reconcile loop (database mode).
...
- **Secrets** — never read `.env`/`.env.local`/`secrets/`/`*.key`/`*.pem`; never git-add them; never embed real values in code or docs. Only placeholders may appear in `.env.example`. Never log `WECOM_ROBOT_ID`, `WECOM_ROBOT_SECRET`, `DIFY_API_KEY`, `WECOM_FORWARD_PLUS_DATABASE_URL` (contains DB credentials), or raw Dify request bodies.
